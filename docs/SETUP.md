...
3.  **Aktifkan Virtual Environment**:
    ...
    - **Mac/Linux**:
      ...
      ```
    *(Tanda berhasil: akan muncul `(venv)` di awal baris terminal)*
...
6.  **Inisialisasi Database & Seeding Data**:
    ...
    - Jalankan script seeding untuk mengisi data awal (Akun Admin & Operator):
      ...
      ```
      *Note: Script ini akan membuat akun admin default dan beberapa data dummy.*
...
- **Solusi**: Pastikan Anda telah melakukan *pull* terbaru atau update kode backend `admin.py`. Masalah ini biasanya terkait data kontainer kosong yang tidak terinisialisasi dengan benar, namun sudah diperbaiki di versi terbaru.
...
2. Jalankan:
   ...
   ```
   *Script ini akan menghapus semua laporan kapal dan log, namun mempertahankan data user (Admin & Operator).*
